...
function Collision.checkCollision(player, World)
    ...
    for i = 1, 6 do hexesToCheck[#hexesToCheck+1] = neighbors[i] end

    local collisions = {}
    local waterTiles = {}
    local onGround = false
    local waterRadius = (player.size / 2) * 0.35

    for _, hex in ipairs(hexesToCheck) do
        local hx, hy = Collision.axialToPixel(hex.q, hex.r)
        local tile      = World.getTile(hex.q, hex.r, player.zLayer)       -- current logical layer
        local tileBelow = World.getTile(hex.q, hex.r, player.zLayer - 1)  -- layer below
        -- 1. Solids on current layer — ALWAYS block
        if tile and tile.type.solid then
            if Collision.circleHexCollision(centerX, centerY, player.size/2, hx, hy) then
                table.insert(collisions, {q=hex.q, r=hex.r, tile=tile, hexCenterX=hx, hexCenterY=hy})
            end
        end
        -- 2. GROUND SUPPORT
        if tileBelow then
            if Collision.pointInHex(feetX, feetY, hx, hy) then
                onGround = true
            end
        end
        -- 3. FALL THROUGH — only if NO tile below AND not supported by current layer jump tile
        if not onGround and not player.isJumping then
            if Collision.circleFullyInsideHex(feetX, feetY, player.size/2, hx, hy) then
                player.zLayer = player.zLayer - 1
            end
        end
        -- 4. ONLY when NOT jumping
        if not player.isJumping then
            -- Water
            if tileBelow and tileBelow.type.isWater then
                if Collision.circleFullyInsideHex(feetX, feetY, waterRadius, hx, hy) then
                    table.insert(waterTiles, {q=hex.q, r=hex.r, tile=tileBelow})
                end
            end
            -- Ledge blocking (walking into air)
            if not tileBelow then
                if Collision.circleHexCollision(centerX, centerY, player.size/2, hx, hy) then
                    table.insert(collisions, {q=hex.q, r=hex.r, isLedge=true, hexCenterX=hx, hexCenterY=hy})
                end
            end
        end
    end
